...
from exercise_code.networks.layer import affine_forward, affine_backward, Sigmoid, Relu
...
class ClassificationNet(Network):
    ...
    def __init__(self, activation=Sigmoid(), num_layer=2,
        ...
        for i in range(num_layer - 2):
            self.params['W' + str(i + 2)] = std * np.random.randn(hidden_size, hidden_size)
            self.params['b' + str(i + 2)] = np.zeros(hidden_size)

        self.params['W' + str(num_layer)] = std * np.random.randn(hidden_size, num_classes)
