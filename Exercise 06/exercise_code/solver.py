...
class Solver(object):
    ...
    def __init__(self, model, train_dataloader, val_dataloader,
        ...
        - train_data_loader: A generator object returning training data
        - val_data_loader: A generator object returning validation data
    ...
    def _step(self, X, y, validation=False):
        ...
        # Compute loss
        loss = self.loss_func.forward(y_predictions, y)
        ...
        if not validation:
            # Compute gradients
            self.opt.backward(y_predictions, y)
            # Update weights
            self.opt.step()
    ...
    def get_dataset_accuracy(self, loader):
        ...
        for batch in loader:
            ...
            y = batch["label"]
            y_predictions = self.model.forward(X)
            label_predictions = np.argmax(y_predictions, axis=1)
            correct += sum(label_predictions == y)
            if y.shape:
                total += y.shape[0]
            else:
                total += 1
