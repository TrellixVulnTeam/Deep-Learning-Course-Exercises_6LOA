...
def batchnorm_backward(d_out, cache):
    ...
    d_var = 0.5 / np.sqrt(var + eps) * d_std
    d_sq = 1.0 / N * np.ones((N, D)) * d_var
    dx_mu2 = 2 * xmu * d_sq
    dx1 = dx_mu1 + dx_mu2
    d_mean = -1.0 * np.sum(dx1, axis=0)
    dx2 = 1.0 / N * np.ones((N, D)) * d_mean
    dx = dx1 + dx2
...
def batchnorm_backward_alt(d_out, cache):
    ...
    """
    dx, d_gamma, d_beta = None, None, None

    ########################################################################
    # TODO: Implement the backward pass for batch normalization. Store the    #
    # results in the dx, d_gamma, and d_beta variables.                         #
    #                                                                         #
    ...
    d_gamma = np.diag(np.dot(d_out.T, x_norm))
    d_beta = np.sum(d_out, axis=0)
